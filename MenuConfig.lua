...
_G.tranTableMC = {
	...
	["Updated"] = "更新日期",
	["current color :"] = "当前颜色:",
	["current key :  SPACEBAR"] = "当前按键:空格",
	["current key :  "] = "当前按键:",
	["current key :  Press new key"] = "当前按键:请按下新按键",
...
function addSettings()
	...
	if _G.AAALoaded then 
    menuconf = MenuConfig("__menuconfig_option__", translationchk("MenuConfig - Settings"))
	else
...
function MenuConfig:InitComponents()
	...
	if _G.AAALoaded then 
    self.OpenMenuButtonText = translationchk("Open MenuConfig")
	else
...
function MenuConfig:DropDown(_header, _text, _value, _droptable, _callback)
	...
	for _, _dropmenu in pairs(_droptable) do
		...
		if _G.AAALoaded then
			dumpchk(_dropmenu)
			_dropmenu = translationchk(_dropmenu)
		end
...
function MenuConfig:Info(_text, _icon)
	...
	if _G.AAALoaded then
		dumpchk(_text)
		for i,v in pairs(_G.tranTableMC) do
			if _text:find(i) then
				_text = _text:gsub(i,v)
			end
		end
	end
...
function MenuConfig:DrawColorPick(text, color, x, y, width, height, selected)
	...
	if selected then
		...
		DrawRectangle(x + width + 10, y, 200, height, ARGB(140,0,0,0))
		if color ~= nil then
			self:DrawTextIcon(nil, tranTableMC["current color :"], MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
			DrawRectangle(30 + x + width + 90, y + 5, 15, 15, ARGB(color[1], color[2], color[3], color[4]))
		else
			self:DrawTextIcon(nil, tranTableMC["current color : NONE"], MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
		end
	end
end
	...
	if selected then
		...
		if key ~= nil and type(key) == "number" then
			if not keychange then
				if key == 32 then
					self:DrawTextIcon(nil, tranTableMC["current key :  SPACEBAR"], MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
				else
					self:DrawTextIcon(nil, tranTableMC["current key :  "] .. string.char(key), MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
				end
			else
				self:DrawTextIcon(nil, tranTableMC["current key :  Press new key"], MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
			end
		else
			if not keychange then
				self:DrawTextIcon(nil, tranTableMC["current key : NONE"], MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
			else
				self:DrawTextIcon(nil, tranTableMC["current key :  Press new key"], MenuTextSize, 30 + x + width, 5 + y, ARGB(255,255,255,255))
			end
		end
